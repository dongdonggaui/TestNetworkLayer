...
- (void)modelCustomTransformFromDictionary:(NSDictionary *)dic {
    ...
    if (_picUrls.count > 0) {
        NSMutableArray *pics = [NSMutableArray new];
        for (NSDictionary *picUrlInfo in _picUrls) {
            NSString *thumbnailUrl = [picUrlInfo objectForKey:@"thumbnail_pic"];
            if (thumbnailUrl) {
                WBPictureMetadata *meta = [[WBPictureMetadata alloc] init];
                meta.url = [NSURL URLWithString:thumbnailUrl];
                WBPicture *pic = [[WBPicture alloc] init];
                pic.thumbnail = meta.copy;
                pic.middlePlus = meta.copy;
                pic.large = meta.copy;
                pic.largest = meta.copy;
                [pics addObject:pic];
            }
        }
        _pics = pics;
