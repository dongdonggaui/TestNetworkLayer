...
// Shop
NSString * const kLHServiceShop = @"kLHServiceShop";

// Weibo
NSString * const kAIFSeviceWeibo = @"kAIFSeviceWeibo";
...
- (AIFService<AIFServiceProtocal> *)newServiceWithIdentifier:(NSString *)identifier
{
    ...
    // Shop
    if ([identifier isEqualToString:kLHServiceShop]) {
        return [[LHShopNetworkService alloc] init];
    }
    
    // Weibo
    if ([identifier isEqualToString:kAIFSeviceWeibo]) {
        return [[AIFServiceWeibo alloc] init];
    }
